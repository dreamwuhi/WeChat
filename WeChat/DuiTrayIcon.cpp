...
#define UIEVENT_TRAYICON WM_USER+1
...
namespace DuiLib
{
	...
	{
		CDuiTrayIcon* ptr = (CDuiTrayIcon*) idEvent;
		ptr->IsVisible() ? ptr->SetHideIcon() : ptr->SetShowIcon();
	}
	...
	bool CDuiTrayIcon::StartTwinkling()
	{
		...
		SetTimer(m_trayData.hWnd,int(this),400,TimerProc);
		m_bTwinkling = true;
		return true;
	}
...
// 			pIDuiTimer = NULL;
// 		}
		KillTimer(m_trayData.hWnd,int(this));
		m_bTwinkling = false;
		SetShowIcon();
	}
